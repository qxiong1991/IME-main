...
parser.add_argument('--IME', action='store_true', help='use IME' )
parser.add_argument('--satellite_UAV_area_ratio_min', default=0, type=float, help='Minimum value of the ratio of the image area of the satellite to that of the UAV')
parser.add_argument('--satellite_UAV_area_ratio_max', default=0, type=float, help='The maximum value of the ratio of the image area of the satellite to that of the UAV')
parser.add_argument('--satellite_UAV_HuMoments2_ratio_min', default=0, type=float, help='Minimum value of the ratio of image moments2')
parser.add_argument('--satellite_UAV_HuMoments2_ratio_max', default=0, type=float, help='The maximum value of the ratio of image moments2')
parser.add_argument('--satellite_UAV_HuMoments_ratio_min', default=0, type=float, help='Minimum value of the ratio of image HuMoments1')
parser.add_argument('--satellite_UAV_HuMoments_ratio_max', default=0, type=float, help='The maximum value of the ratio of image HuMoments1')
parser.add_argument('--ProjectedModel', default='sdddddddddddddd', type=str, help='Model name' )    
parser.add_argument('--output_file', default='sssddf', type=str, help='output file name' ) 
...
def csv_write_image_location(photo):
    ...
    header = ['Filename', 'Latitude', 'Longitude', 'Calculated_Latitude', 'Calculated_Longitude', 'Latitude_Error', 'Longitude_Error', 'Meters_Error', 'Corrected', 'Matched','uname']
    with open(output_file, 'a', encoding='UTF8') as f:
        ...
        if filesize == 0:
            ...
            #dist_error =  distance.distance(loc1,loc2).m
...
if __name__ == '__main__':
    ...
    for drone_image in drone_images_list:
        ...
        photo =  cv2.imread(drone_image.filename) # read the drone image   #(3040, 4056, 3)
        ...
        for rot in rotations:
            ...
            ProjectedModel = opt.ProjectedModel

            #Call superglue wrapper function to match the query image to the map
            satellite_map_index_new, center_new, located_image_new, features_mean_new, query_image_new, feature_number,satellite_UAV_area_ratio ,satellite_UAV_HuMoments_ratio,satellite_UAV_HuMoments2_ratio  = superglue_utils.match_image(ProjectedModel )
            ...
            IME = opt.IME
            satellite_UAV_area_ratio_min = opt.satellite_UAV_area_ratio_min
            satellite_UAV_area_ratio_max = opt.satellite_UAV_area_ratio_max
            satellite_UAV_HuMoments_ratio_min = opt.satellite_UAV_HuMoments_ratio_min
            satellite_UAV_HuMoments_ratio_max = opt.satellite_UAV_HuMoments_ratio_max
            satellite_UAV_HuMoments2_ratio_min = opt.satellite_UAV_HuMoments2_ratio_min
            satellite_UAV_HuMoments2_ratio_max = opt.satellite_UAV_HuMoments2_ratio_max
            if IME :
                if satellite_UAV_area_ratio_min <= satellite_UAV_area_ratio <= satellite_UAV_area_ratio_max and satellite_UAV_HuMoments_ratio_min <= satellite_UAV_HuMoments_ratio  <= satellite_UAV_HuMoments_ratio_max and satellite_UAV_HuMoments2_ratio_min <= satellite_UAV_HuMoments2_ratio  <= satellite_UAV_HuMoments2_ratio_max :   #最佳方法   但是对应的数据不是最佳
        ...
        satellite_map_index_new_0 = satellite_map_index_new
        center_new_0 = center_new
        located_image_new_0 = located_image_new
